...
#### 引导弹窗重复弹出
- **问题**：用户点击"保留本窗口"后，弹窗关闭又立刻重新弹出
- **根因**：`dismissGuide()` 将 `hasActiveGuideResolve` 重置为 false，但另一个窗口仍在注册表中，5秒后多窗口检测条件再次满足
- **修复**：用冷却期机制（60秒）替代简单的布尔标志重置。用户做出选择后记录 `lastGuideResolvedAt`，冷却期内不再触发引导弹窗

#### 后台标签页误触发多窗口引导
- **问题**：用户只有前台一个窗口在看视频，但后台标签页的心跳还在更新，`hasMixedWindowTypes()` 误判为混合类型，触发引导弹窗
- **根因**：多窗口检测没有判断当前窗口是否在前台（visible），导致后台标签页的注册信息干扰检测
- **修复**：多窗口检测增加 `!document.hidden` 条件，当前窗口在后台时不触发引导

#### 勾选白名单 + 保留本窗口无反应
- **问题**：用户勾选"将当前视频添加到学习白名单"后点击"保留本窗口"，看起来无反应
- **根因**：实际上是 Bug 1 的连锁反应——弹窗关了又立刻弹出来，视觉上像"无反应"
- **修复**：Bug 1 修复后此问题自然解决。额外增加：白名单添加成功后显示 Toast 反馈「✅ 已将「xxx」添加到学习白名单」，并重置干预状态

#### 倒计时卡在某个数字不动
- **问题**：30秒倒计时走到某个数字后不再递减
- **根因**：Bug 1 导致弹窗被重复 dismiss/create，倒计时 interval 被 clear
- **修复**：Bug 1 修复后此问题自然解决

### 暗色适配补全 🎨

- **overlay 背景**：学习窗口叠加层暗色模式加深（`rgba(0,0,0,0.65)`）
- **backdrop 背景**：引导弹窗背景暗色模式加深
- **badge 学习标签**：暗色下半透明绿色底 + 亮绿色文字
- **badge 非学习标签**：暗色下半透明红色底 + 亮红色文字
- **btn-primary 按钮**：暗色下从刺眼纯红改为暗红（`#b91c1c`）+ 浅红色文字
- **checkbox**：暗色下 accent-color 改为 `#60a5fa`
- **学习窗口叠加层**：创建时自动检测并应用暗色主题

### 调试增强 🔧

- 多窗口检测定时器：增加 `activeTabs` 详细信息、`inCooldown` 状态、`documentHidden` 状态日志
- `hasMixedWindowTypes()`：输出哪些窗口被判定为 study/distraction
- `visibilitychange`/`focus` 事件：增加详细状态变更日志
- `updateRegistration()`：节流日志（每30秒一次），输出当前注册状态
- `_handleGuideChoice()`：输出选择详情、白名单添加结果
- `dismissGuide()`：输出冷却期截止时间

---

## [1.2.6] - 2026-04-22

### 多窗口 Master 选举 + 引导弹窗 🖥️

#### TabManager 模块（新增）
- **Master 选举机制**：谁被用户关注（visible/focus），谁是 Master
- **心跳机制**：Master 每 3 秒写心跳，非 Master 每 3 秒检查，8 秒无心跳自动接管
- **标签页注册表**：localStorage `bilibiliStudy_tabRegistry`，记录所有活跃标签页信息
- **可见性监听**：`visibilitychange`（hidden→释放, visible→接管）+ `focus`（抢夺）+ `beforeunload`（释放+注销）
- **最小持有时间**：Master 抢到后至少 3 秒不可释放，防止并排窗口频繁切换
- **同类型不弹引导**：两个学习窗口或两个分心窗口并存时，不弹出引导弹窗（自动让先开的当Master）

#### 主定时器改造
- **Master 分支**：正常计时 + `InterventionController.check()` + 同步全局状态
- **非 Master 分支**：只更新本地状态，不写统计、不弹弹窗
- **多窗口暂停**：引导弹窗激活期间，所有窗口暂停计时

#### 多窗口引导弹窗
- **触发条件**：检测到不同类型窗口并存（1学+1分）时触发
- **学习窗口**：显示高斯模糊叠加层 + 标注文字"检测到多个学习窗口，计时已暂停"
- **分心窗口**：弹出引导弹窗，显示所有窗口列表、30秒倒计时、白名单快速添加
- **倒计时结束**：默认分心窗口成为 Master，继承干预状态；学习窗口标注"已离开学习状态"
- **用户选择**：
  - 关闭本窗口 → 记录 BV 号到历史，尝试 `window.close()`
  - 保留本窗口 → 成为 Master，恢复计时
- **白名单快捷添加**：引导弹窗内可勾选"将当前视频添加到学习白名单"

#### 浮窗多窗口标识
- **🟢 主窗口**：正常绿色/红色背景，正常计时
- **🔵 副窗口**：钢蓝色（学习中）/ 灰色（暂停），显示"副窗口"/"已暂停"
- **⏸️ 协商中**：灰色背景 + 脉冲动画，显示"计时暂停 - 多窗口协商中"

#### ModalManager 扩展
- 新增 `MULTI_TAB: 7` 层级（最高优先级），可覆盖一切弹窗
- 引导弹窗注册为 `multi-tab-guide`

#### CSS 新增
- `.bilibili-study-multi-tab-overlay`：学习窗口高斯模糊叠加层
- `.bilibili-study-multi-tab-guide`：分心窗口引导弹窗（含 backdrop/content/header/body/footer）
- `.bilibili-study-multi-tab-item`：窗口列表项（学习/分心/当前 不同配色）
- `.bilibili-study-floating-secondary/negotiating`：浮窗副窗口/协商中样式
- 所有新增组件完整暗色模式适配
